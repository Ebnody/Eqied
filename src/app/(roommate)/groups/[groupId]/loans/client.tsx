...
export default function GroupLoansClient({
  initialLoans,
  initialMembers,
}: {
  initialLoans: Loan[];
  ...
  const [form, setForm] = useState({
    lenderMemberId: initialMembers[0]?.id ?? "",
    borrowerMemberId: initialMembers[1]?.id ?? initialMembers[0]?.id ?? "",
    amountEtb: "",
    reason: "",
  });
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch(`/api/roommate/groups/${groupId}/loans`, {
      method: "POST",
  ...
  async function recordPayment(loanId: string) {
    ...
    const res = await fetch(`/api/roommate/groups/${groupId}/loans/${loanId}/payment`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-100">{t("roommate.page.loans")}</h2>
      ...
      {loans.length === 0 ? (
        <Card className="border-white/10">
          <CardContent className="p-8 text-center text-slate-400 text-sm">{t("roommate.page.noLoans")}</CardContent>
        </Card>
      ) : (
        <Card className="overflow-hidden border-white/10">
          {loans.map((l) => {
            ...
            return (
              <div key={l.id} className="px-4 py-3 border-b border-white/10 last:border-b-0 hover:bg-white/5 transition-colors">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-slate-200">{l.lender.user.fullName} → {l.borrower.user.fullName}</p>
                    <p className="text-xs text-slate-500">{l.reason || "—"} · {new Date(l.occurredAt).toLocaleDateString()}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-slate-300">{formatETB(l.amount)}</p>
