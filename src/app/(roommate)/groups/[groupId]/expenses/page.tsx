...
export default function GroupExpensesPage() {
  ...
  const [form, setForm] = useState({
    ...
    amountEtb: "",
    categoryKey: "rent",
    splitType: "equal" as "equal" | "percent" | "exact",
    paidByMemberId: "",
    notes: "",
  ...
  async function load() {
    ...
    if (gRes.members?.length && !form.paidByMemberId) {
      setForm((f) => ({ ...f, paidByMemberId: gRes.members[0].id, participantIds: gRes.members.map((m: Member) => m.id) }));
    }
    setLoading(false);
  }
  ...
  useEffect(() => {
    ...
    es.addEventListener("expense_added", () => load());
    return () => es.close();
  }, [groupId]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-slate-800">{t("roommate.page.expenses")}</h2>
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <select id="splitType" value={form.splitType} onChange={(e) => setForm((f) => ({ ...f, splitType: e.target.value as any }))}
                    className="flex h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-1">
                ...
                {form.splitType !== "equal" && (
                  <div className="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {form.participantIds.map((id) => {
                      ...
                      return (
                        <div key={id}>
                          <Label className="text-xs text-slate-500">{m?.user.fullName ?? m?.user.telegramUsername}</Label>
      ...
      {expenses.length === 0 ? (
        ...
        <Card className="overflow-hidden">
          {expenses.map((e) => (
            <div key={e.id} className="px-4 py-3 border-b last:border-b-0 hover:bg-slate-50">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-slate-700 shrink-0 ml-3">{formatETB(e.amount)}</span>
              </div>
